...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       # \DONE
       (gmm_verif -d $w/$FEAT -e $FEAT -D $w/gmm/$FEAT -E gmm -w $world $lists/gmm.list  $lists/verif/all.test $lists/verif/all.test.candidates | tee $w/verif_${FEAT}_${name_exp}.log) || exit 1
