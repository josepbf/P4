...
compute_mfcc() {
    ...
    for filename in $(sort $*); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2mfcc 30 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           # Es put augmentar -N (iteracciones) i -m (gaussianas), i disminuir -T a 0.0001
           gmm_train  -v 1 -m 128 -N30 -T 1e-6 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
       ...
       gmm_train  -v 1 -T 1e-6 -N30 -m 128 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
       ...
       (gmm_verify -d $w/$FEAT -e $FEAT -D $w/gmm/$FEAT -E gmm $lists/gmm.list -w $world $lists/verif/all.test $lists/verif/all.test.candidates | tee $w/verif_${FEAT}_${name_exp}.log) || exit 1
       ...
       (gmm_verify -d $w/$FEAT -e $FEAT -D $w/gmm/$FEAT -E gmm -w $world $lists/gmm.list  $lists/final/verif.test $lists/final/verif.test.candidates | tee $w/verif_test.log) || exit 1
        perl -ane 'print "$F[0]\t$F[1]\t";
        if ($F[2] > -0.986715304811012) {print "1\n"}
        else {print "0\n"}' $w/verif_test.log > verif_test.log
